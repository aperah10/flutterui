  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(
          bottom: ScrSizeConfig.heightMultiplier != null
              ? 4 * ScrSizeConfig.heightMultiplier
              : 15.0),
      child: TextField(
        obscureText: false,
        decoration: InputDecoration(
            ...
            hintStyle: TextStyle(
              fontSize: ScrSizeConfig.heightMultiplier > 7.68
                  ? 1.3 * ScrSizeConfig.heightMultiplier
                  : 2.4 * ScrSizeConfig.heightMultiplier,
              color: Colors.black,
            )),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return MaterialButton(
      ...
      padding: EdgeInsets.symmetric(
          horizontal: ScrSizeConfig.heightMultiplier != null
              ? 5 * ScrSizeConfig.heightMultiplier
              : 15.0),
      shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(
              ScrSizeConfig.heightMultiplier != null
                  ? 5 * ScrSizeConfig.heightMultiplier
                  : 15.0)),
      onPressed: () {},
      child: Text(btnName,
          style: TextStyle(
              fontSize: ScrSizeConfig.heightMultiplier != null
                  ? 2.5 * ScrSizeConfig.heightMultiplier
                  : 15.0,
              letterSpacing: 2.2,
              color: Colors.black)),
    );
